...
namespace Benchmarks
{
    public class GuidBenchmarks
    {
        ...
        private void LogDetailedResults(string collectionType, int n, int lookupCount, TimeSpan creationTime, TimeSpan lookupTime, TimeSpan totalTime)
        {
            ...
            var logPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "result", "detailed_results_guid.txt");
            ...
            lock (_fileLock)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    var backupPath = Path.Combine(Path.GetTempPath(), "benchmark_debug_guid.txt");
