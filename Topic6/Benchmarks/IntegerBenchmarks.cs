...
namespace Benchmarks
{
    public class IntegerBenchmarks
    {
        ...
        private void LogDetailedResults(string collectionType, int n, int lookupCount, TimeSpan creationTime, TimeSpan lookupTime, TimeSpan totalTime)
        {
            ...
            var logPath = Path.Combine(projectDirectory, "result", $"detailed_results_integer_{Priority}.txt");
            ...
            lock (_fileLock)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    var backupPath = Path.Combine(Path.GetTempPath(), $"benchmark_debug_integer_{Priority}.txt");
